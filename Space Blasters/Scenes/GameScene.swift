...
//        enemyBossClass = EnemyBoss()
    ...
    func startLevel () {
        ...
        let enemShip = enemyShipsClassInstance
        enemShip.gameSceneClass = self
        let enemBossShip = enemyBossClassInstance
        enemBossShip.gameSceneClass = self
        
        
        let spawn = SKAction.run(enemShip.objectSetUp)
        let spawnEnemyBosses = SKAction.run(enemBossShip.objectSetUp)
